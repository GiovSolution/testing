...
/*			$limit = $query." LIMIT ".$start.",".$end;			
	...
	// Fungsi utk melakukan update/insert hutang
	function catatan_hutang_update($order_create_id){
		...
		if($rs->num_rows()){
			...
			$hutang_jatuhtempo=$rs_record["hutang_jatuhtempo"];
			...
			/* untuk itu, No.Faktur ini akan dimasukkan ke db.hutang sebagai daftar hutang nopi untuk dilunasi nantinya */
				...
				"hutang_jatuhtempo"=>$hutang_jatuhtempo,
		...
		function master_order_beli_update($order_id ,$order_no ,$order_supplier ,$order_tanggal ,$order_carabayar ,$order_diskon, $order_cashback ,
										  $order_biaya , $order_totalbiaya, $order_bayar ,$order_keterangan, $order_status, $order_status_acc, $cetak_order){
			$data = array(
				"order_id"=>$order_id, 
				"order_no"=>$order_no, 
		...
		function master_order_beli_create($order_no ,$order_supplier ,$order_tanggal ,$order_carabayar ,$order_diskon, $order_cashback ,$order_biaya , $order_totalbiaya, $order_bayar ,$order_keterangan, $order_status, $order_status_acc, $cetak_order){
			$date_now=date('Y-m-d');
			//if($order_tanggal==""){
			...
			if($cetak_order == 1)
			{
				$this->catatan_hutang_update($temp_id);
				return $temp_id;
			}
			else{
				return $temp_id;
