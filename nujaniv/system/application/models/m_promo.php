...
class M_promo extends Model{
		...
		function promo_list($filter,$start,$end){
			$date_now=date('m-d');
			$date_now_com=date('Y-m-d');

			$query = "SELECT customer.* , 
						date_format('".$date_now_com."','%Y') - date_format(cust_tgllahir,'%Y') as umur
						from customer
						where date_format(cust_tgllahir,'%m-%d') > '".$date_now."' and date_format(cust_tgllahir,'%m-%d') < DATE_FORMAT(date_add('".$date_now_com."', interval 7 day),'%m-%d') 
			";
