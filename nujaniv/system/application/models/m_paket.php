...
//				"paket_dm"=>$paket_dm,
		...
		$sql_paket_awal = "SELECT kategori_id FROM kategori WHERE kategori_id='$paket_group'";
			$rs_paket_awal = $this->db->query($sql_paket_awal);
			if($rs_paket_awal->num_rows()){
				$data["paket_kategori"]=$paket_group;
				//autogenerate kodebaru
				$sql = "SELECT kategori_id, kategori_keterangan FROM kategori WHERE kategori_id='".$paket_group."'";
				$rs = $this->db->query($sql);
				if($rs->num_rows()){
					$row = $rs->row_array();
					$group_kode = $row['kategori_keterangan'];
				}		
