...
class M_master_lunas_hutang extends Model{
	...
	function get_faktur_hutang_selected_list($fhutang_id,$query,$start,$end){
		if($fhutang_id>0){
			$sql_dpiutang="SELECT dhutang_master, master_lunas_hutang.fhutang_id as fhutang_id
		...
		$sql="SELECT fhutang_id
				,fhutang_nobukti
				,fhutang_cust
				,date_format(fhutang_tanggal,'%d-%m-%Y') AS fhutang_tanggal
				,fhutang_bayar
				,fhutang_cara
				,fhutang_stat_dok
				,hutang.hutang_faktur as hutang_faktur
				,hutang.hutang_id as hutang_id
			FROM detail_lunas_hutang
		...
		if($rs_rows){
			...
			foreach($rs->result() as $row_dpiutang){
				...
				$filter.="OR fhutang_id='".$row_dpiutang->fhutang_id."' ";
			}
	...
	function detail_lunas_hutang_insert($fpiutang_cust ,$array_dpiutang_id ,$array_lpiutang_id , $array_hutang_op_id, $fhutang_id ,$array_dpiutang_nilai
		...
		for($i = 0; $i < sizeof($array_lpiutang_id); $i++){
			...
			$hutang_op_id = $array_hutang_op_id[$i];
			...
			$temp_dhutang_id = $this->db->insert_id();
			...
			if($cetak_lp==1 && $i==$size_array){
				...
				$sql = "SELECT * FROM vu_hutang_total_lunas WHERE vu_hutang_total_lunas.dhutang_id='".$temp_dhutang_id."'";
				$rs = $this->db->query($sql);
				if($rs->num_rows()>0){
					foreach($rs->result() as $row){
						$sqlu = "UPDATE hutang
							SET hutang_sisa = (hutang_sisa-".$row->total_pelunasan.")
			...
			/*
				...
				if($rs->num_rows()>0){
					foreach($rs->result() as $row){
						...
						$sqlu_status = "UPDATE hutang
							SET hutang_status = 'lunas', hutang_stat_dok = 'Tertutup'
							WHERE hutang_sisa=0 AND hutang_id=".$row->dhutang_hutang;
						$this->db->query('LOCK TABLE hutang WRITE');
						$this->db->query($sqlu_status);
						$this->db->query('UNLOCK TABLES');
					}
	...
	function master_lunas_hutang_update($fhutang_id ,$fpiutang_no ,$fpiutang_cust ,$fpiutang_tanggal, $fpiutang_keterangan ,$fpiutang_status
										...
										,$array_dpiutang_id ,$array_lpiutang_id , $array_hutang_op_id, $array_dpiutang_nilai ,$array_dpiutang_keterangan
										,$cetak_lp){
		$date_now=date('Y-m-d');
		...
		if($rs>-1){
			...
			$rs_dpiutang_insert = $this->detail_lunas_hutang_insert($fpiutang_cust ,$array_dpiutang_id ,$array_lpiutang_id , $array_hutang_op_id,$fhutang_id ,$array_dpiutang_nilai , $fpiutang_cara, $array_dpiutang_keterangan ,$cetak_lp);
	...
	function master_lunas_hutang_create($fpiutang_cust ,$fpiutang_tanggal, $fpiutang_keterangan ,$fpiutang_status
		...
		if($rs>0){
			...
			$rs_dpiutang_insert = $this->detail_lunas_hutang_insert($fpiutang_cust ,$array_dpiutang_id ,$array_lpiutang_id , $array_hutang_op_id, $fhutang_id
	...
	function master_lunas_hutang_batal($fhutang_id, $fpiutang_tanggal){
		...
		$datetime_now = date('Y-m-d H:i:s');
		$sql = "UPDATE master_lunas_hutang
			SET fhutang_stat_dok='Batal'
				,fhutang_update='".@$_SESSION[SESSION_USERID]."'
				,fhutang_date_update='".$datetime_now."'
				,fhutang_revised=fhutang_revised+1
			WHERE fhutang_id='".$fhutang_id."' " ;
		...
		if($rs>0){
			...
			$sql = "SELECT dhutang_master, fhutang_nobukti, fhutang_cara, dhutang_nilai, dhutang_hutang
				FROM detail_lunas_hutang
					LEFT JOIN master_lunas_hutang on (master_lunas_hutang.fhutang_id = detail_lunas_hutang.dhutang_master)
				WHERE fhutang_id='".$fhutang_id."'";
			$rs = $this->db->query($sql);
			if($rs->num_rows()>0){
				$record = $rs->row_array();
				$fpiutang_nobukti = $record['fhutang_nobukti'];
				$fpiutang_cara = $record['fhutang_cara'];
				...
				foreach($rs->result() as $row){
					...
					$sqlu = "UPDATE hutang
							SET hutang_sisa = (hutang_sisa+".$row->dhutang_nilai.")
							WHERE hutang_id=".$row->dhutang_hutang;
						// $this->db->query('LOCK TABLE hutang WRITE');
						$this->db->query($sqlu);
						// $this->db->query('UNLOCK TABLES');
