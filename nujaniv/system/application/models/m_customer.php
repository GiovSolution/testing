...
/*			$limit = $sql." LIMIT ".$start.",".$end;		
		...
		function customer_list($filter,$start,$end){
			$query =   "
					select `customer`.`cust_id` AS `cust_id`,`customer`.`cust_no` AS `cust_no`,`customer`.`cust_member` AS `cust_member`,(case when (length(`member`.`member_no`) = 14) then concat(substr(`member`.`member_no`,1,2),'-',substr(`member`.`member_no`,3,6),'-',substr(`member`.`member_no`,9)) else concat(substr(`member`.`member_no`,1,6),'-',substr(`member`.`member_no`,7,6),'-',substr(`member`.`member_no`,13)) end) AS `member_no`,`member`.`member_register` AS `member_register`,`member`.`member_valid` AS `member_valid`,`customer`.`cust_nama` AS `cust_nama`,`customer`.`cust_title` AS `cust_title`,`customer`.`cust_panggilan` AS `cust_panggilan`,`customer`.`cust_foreigner` AS `cust_foreigner`,`customer`.`cust_kelamin` AS `cust_kelamin`,`customer`.`cust_alamat` AS `cust_alamat`,`customer`.`cust_kota` AS `cust_kota`,`customer`.`cust_kodepos` AS `cust_kodepos`,`customer`.`cust_propinsi` AS `cust_propinsi`,`customer`.`cust_negara` AS `cust_negara`,`customer`.`cust_alamat2` AS `cust_alamat2`,`customer`.`cust_kota2` AS `cust_kota2`,`customer`.`cust_kodepos2` AS `cust_kodepos2`,`customer`.`cust_propinsi2` AS `cust_propinsi2`,`customer`.`cust_negara2` AS `cust_negara2`,`customer`.`cust_telprumah` AS `cust_telprumah`,`customer`.`cust_telprumah2` AS `cust_telprumah2`,`customer`.`cust_telpkantor` AS `cust_telpkantor`,`customer`.`cust_hp` AS `cust_hp`,`customer`.`cust_hp2` AS `cust_hp2`,`customer`.`cust_hp3` AS `cust_hp3`,`customer`.`cust_email` AS `cust_email`,`customer`.`cust_fb` AS `cust_fb`,`customer`.`cust_tweeter` AS `cust_tweeter`,`customer`.`cust_email2` AS `cust_email2`,`customer`.`cust_fb2` AS `cust_fb2`,`customer`.`cust_tweeter2` AS `cust_tweeter2`,`customer`.`cust_agama` AS `cust_agama`,`customer`.`cust_pendidikan` AS `cust_pendidikan`,`customer`.`cust_profesi` AS `cust_profesi`,`customer`.`cust_tmptlahir` AS `cust_tmptlahir`,`customer`.`cust_tgllahir` AS `cust_tgllahir`,(date_format(now(),_utf8'%Y') - date_format(`customer`.`cust_tgllahir`,_utf8'%Y')) AS `cust_umur`,`customer`.`cust_hobi` AS `cust_hobi`,`customer`.`cust_referensi` AS `cust_referensi`,`customer`.`cust_referensilain` AS `cust_referensilain`,`customer`.`cust_keterangan` AS `cust_keterangan`,`customer`.`cust_terdaftar` AS `cust_terdaftar`,`customer`.`cust_statusnikah` AS `cust_statusnikah`,`customer`.`cust_jmlanak` AS `cust_jmlanak`,`customer`.`cust_unit` AS `cust_unit`,`customer`.`cust_cp` AS `cust_cp`,`customer`.`cust_cptelp` AS `cust_cptelp`,`customer`.`cust_aktif` AS `cust_aktif`,`customer`.`cust_fretfulness` AS `cust_fretfulness`,`customer`.`cust_point` AS `cust_point`,`customer`.`cust_creator` AS `cust_creator`,`customer`.`cust_date_create` AS `cust_date_create`,`customer`.`cust_update` AS `cust_update`,`customer`.`cust_date_update` AS `cust_date_update`,`customer`.`cust_revised` AS `cust_revised`,`cust_ref`.`cust_nama` AS `cust_nama_ref`,`cabang`.`cabang_nama` AS `cabang_nama`,`cabang`.`cabang_alamat` AS `cabang_alamat`,`customer`.`cust_crm_value` AS `cust_crm_value`,`crm_value`.`crmvalue_date` AS `crmvalue_date`,`crm_value`.`crmvalue_total` AS `crmvalue_total`,`customer`.`cust_priority` AS `cust_priority`,`customer`.`cust_bb` AS `cust_bb`,`customer`.`cust_tglawaltrans` AS `cust_tglawaltrans`,(case `customer`.`cust_priority` when 'High' then '*' else '' end) AS `cust_priority_star` 
						from `customer` 
						left join `customer` `cust_ref` on `customer`.`cust_referensi` = `cust_ref`.`cust_id`
						left join `cabang` on `customer`.`cust_unit` = `cabang`.`cabang_id`
						left join `member` on `member`.`member_id` = `customer`.`cust_member`
						left join `crm_value` on `customer`.`cust_crm_value` = `crm_value`.`crmvalue_id`
						WHERE `customer`.cust_aktif = 'Aktif' 
						";
			
			// For simple search
			if ($filter<>""){
				$query .=eregi("WHERE",$query)? " AND ":" WHERE ";
				$query .= " (customer.cust_no LIKE '%".addslashes($filter)."%' OR customer.cust_nama LIKE '%".addslashes($filter)."%' OR customer.cust_alamat LIKE '%".addslashes($filter)."%' OR customer.cust_alamat2 LIKE '%".addslashes($filter)."%' OR customer.cust_telprumah LIKE '%".addslashes($filter)."%' OR customer.cust_telprumah2 LIKE '%".addslashes($filter)."%' OR customer.cust_telpkantor LIKE '%".addslashes($filter)."%' OR customer.cust_hp LIKE '%".addslashes($filter)."%' OR customer.cust_hp2 LIKE '%".addslashes($filter)."%' OR customer.cust_hp3 LIKE '%".addslashes($filter)."%' OR customer.cust_member LIKE '%".addslashes($filter)."%' OR member_no LIKE '%".addslashes($filter)."%' )";
				$query .= " AND customer.cust_aktif = 'Aktif'"; // by hendri, simple search khusus aktif only
			}
			
			$query .= " order by customer.cust_id desc ";
