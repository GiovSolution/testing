...
class M_master_retur_beli extends Model{
		...
		function get_laporan($tgl_awal,$tgl_akhir,$periode,$opsi,$group,$faktur){
			...
			switch($group){
				...
				case "Supplier": $order_by=" ORDER BY supplier_nama";break;
		...
		function get_terima_list($query,$start,$end){
			$sql="SELECT master_terima_beli.terima_id , master_terima_beli.terima_no, master_terima_beli.terima_tanggal,
						supplier.supplier_nama,
						master_order_beli.order_no
				from master_terima_beli
				left join supplier on (master_terima_beli.terima_supplier = supplier.supplier_id)
		...
		function get_produk_all_list($terima_id, $selected_id, $query,$start,$end){
			...
			$sql="SELECT distinct produk.produk_id,produk.produk_nama,produk.produk_kode,
							satuan.satuan_id,satuan.satuan_nama,
							detail_terima_beli.dterima_harga as dinvoice_harga, detail_terima_beli.dterima_diskon as dinvoice_diskon 
					FROM detail_terima_beli
					LEFT JOIN master_terima_beli on (master_terima_beli.terima_id = detail_terima_beli.dterima_master) 
					LEFT JOIN produk on (produk.produk_id = detail_terima_beli.dterima_produk)
					LEFT JOIN satuan on (satuan.satuan_id = detail_terima_beli.dterima_satuan)
					WHERE master_terima_beli.terima_id='".$terima_id."'";
