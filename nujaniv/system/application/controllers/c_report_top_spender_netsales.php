...
//class of tindakan
class C_report_top_spender_netsales extends Controller {

	//constructor
	function C_report_top_spender_netsales(){
		parent::Controller();
	...
	function index(){
		...
		$this->load->view('main/v_report_top_spender_netsales');
	}
	...
	function get_action(){
		...
		switch($task){
			case "SEARCH":
				$this->top_spender_netsales_search();
				break;
			case "PRINT":
				$this->tindakan_print();
				break;
	...
	function top_spender_netsales_search(){
		...
		$umur_start	= trim(@$_POST["umur_start"]);
		$umur_end	= trim(@$_POST["umur_end"]);
		
		$result = $this->m_report_top_spender_netsales->top_spender_netsales_search($tgl_start, $tgl_end, $top_jenis, $top_jumlah, $umur_start, $umur_end);
		echo $result;
	}
	...
	function tindakan_export_excel(){
		...
		$query = $this->m_report_top_spender_netsales->tindakan_export_excel($trawat_id, $trawat_tglapp_start ,$trawat_tglapp_end, $top_jenis ,$top_jumlah ,$option,$filter);

		to_excel($query,"top_spender_netsales"); 
		echo '1';
