...
class C_master_mutasi extends Controller {
	...
	function get_mutasi_order_detail_by_order_id(){
		$orderid = isset($_POST['orderid']) ? @$_POST['orderid'] : "";
		$result=$this->m_master_mutasi->get_mutasi_order_detail_by_order_id($orderid);
		echo $result;
	}
	
	function get_order_mutasi_list(){
		$query = isset($_POST['query']) ? @$_POST['query'] : "";
		$start = (integer) (isset($_POST['start']) ? @$_POST['start'] : @$_GET['start']);
		$end = (integer) (isset($_POST['limit']) ? @$_POST['limit'] : @$_GET['limit']);
		
		$result=$this->m_master_mutasi->get_order_mutasi_list($query,$start, $end);
		echo $result;
	}
	...
	function print_laporan(){
		...
		if($opsi=='rekap'){
			...
			switch($group){
				...
				case "Produk Non Racikan(Retail)": $print_view=$this->load->view("main/p_rekap_mutasi_produk_non_racikan.php",$data,TRUE);break;
	...
	function get_produk_list(){
		...
		$no_ref = isset($_POST['no_ref']) ? @$_POST['no_ref'] : @$_GET['no_ref'];
		if($task=='detail')
			$result=$this->m_master_mutasi->get_produk_detail_list($master_id,$query,$start,$end);
		elseif($task=='list')
			$result=$this->m_master_mutasi->get_produk_all_list($query,$start,$end);
		elseif($task=='selected')
			$result=$this->m_master_mutasi->get_produk_selected_list($master_id, $selected_id,$query,$start,$end);
		elseif($task=='order')
			$result=$this->m_master_mutasi->get_produk_order_list($order_id,$query,$start,$end);
		elseif($task=='no_ref')
			$result=$this->m_master_mutasi->get_produk_no_ref_list($no_ref, $query,$start,$end);
	...
	function get_satuan_list(){
		...
		elseif($task=='selected')
			$result=$this->m_master_mutasi->get_satuan_selected_list($selected_id);
		elseif($task=='order')
			$result=$this->m_master_mutasi->get_satuan_order_list($order_id);
		elseif($task=='all')
			$result=$this->m_master_mutasi->get_satuan_all_list();	
		echo $result;
	}
	...
	function master_mutasi_update(){
		...
		$mutasi_spb_id=trim(@$_POST["mutasi_spb_id"]);
		$mutasi_spb_id=str_replace("/(<\/?)(p)([^>]*>)", "",$mutasi_spb_id);
		$mutasi_spb_id=str_replace("'", '"',$mutasi_spb_id);
		...
		$mutasi_tujuan_id=trim(@$_POST["mutasi_tujuan_id"]);
		$mutasi_asal_id=trim(@$_POST["mutasi_asal_id"]);
		...
		$result = $this->m_master_mutasi->master_mutasi_update($mutasi_id ,$mutasi_no, $mutasi_spb_id,$mutasi_spb, $mutasi_asal ,$mutasi_tujuan ,$mutasi_tanggal ,$mutasi_keterangan, $mutasi_status, $mutasi_status_terima, $mutasi_barang_keluar, $mutasi_racikan, $mutasi_kategori_barang_keluar, $cetak, $printonly,$mutasi_tujuan_id,$mutasi_asal_id);
		echo $result;
	}
	...
	function master_mutasi_create(){
		...
		//Data Detail Produk Satuan
		/*$dmutasi_id = $_POST['dmutasi_id']; // Get our array back and translate it :
		$array_dmutasi_id = json_decode(stripslashes($dmutasi_id));
		
		$dmutasi_produk = $_POST['dmutasi_produk']; // Get our array back and translate it :
		$array_dmutasi_produk = json_decode(stripslashes($dmutasi_produk));
		
		$dmutasi_satuan = $_POST['dmutasi_satuan']; // Get our array back and translate it :
		$array_dmutasi_satuan = json_decode(stripslashes($dmutasi_satuan));
		
		$dmutasi_jumlah = $_POST['dmutasi_jumlah']; // Get our array back and translate it :
		$array_dmutasi_jumlah = json_decode(stripslashes($dmutasi_jumlah));*/
		...
		$array_dmracikan_id, $array_dmracikan_produk, $array_dmracikan_satuan, $array_dmracikan_jumlah,$mutasi_tujuan_id,$mutasi_asal_id);
		echo $result;
	}
