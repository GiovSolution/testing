...
class C_karyawan extends Controller {
	...
	function detail_jabatan_delete(){
        $kjabatan_id = trim(@$_POST["kjabatan_id"]); // Get our array back and translate it :
		$result=$this->m_karyawan->detail_jabatan_delete($kjabatan_id);
		echo $result;
    }
	...
	// GAJI
	// list detail gaji
	function detail_gaji_list(){
		$query = isset($_POST['query']) ? $_POST['query'] : "";
		$start = (integer) (isset($_POST['start']) ? $_POST['start'] : $_GET['start']);
		$end = (integer) (isset($_POST['limit']) ? $_POST['limit'] : $_GET['limit']);
		$master_id = (integer) (isset($_POST['master_id']) ? $_POST['master_id'] : $_GET['master_id']);
		$result=$this->m_karyawan->detail_gaji_list($master_id,$query,$start,$end);
		echo $result;
	}
	...
	function detail_gaji_insert(){
		//POST variable here
		$kgaji_id = $_POST['kgaji_id']; // Get our array back and translate it :
		$array_kgaji_id = json_decode(stripslashes($kgaji_id));
		
		$kgaji_master=trim(@$_POST["kgaji_master"]);
			
		$kgaji_tanggal = $_POST['kgaji_tanggal']; // Get our array back and translate it :
		$array_kgaji_tanggal = json_decode(stripslashes($kgaji_tanggal));
		
		$kgaji_nominal = $_POST['kgaji_nominal']; // Get our array back and translate it :
		$array_kgaji_nominal = json_decode(stripslashes($kgaji_nominal));
		
		$kgaji_keterangan = $_POST['kgaji_keterangan']; // Get our array back and translate it :
		$array_kgaji_keterangan = json_decode(stripslashes($kgaji_keterangan));
		
		$result=$this->m_karyawan->detail_gaji_insert($array_kgaji_id , $kgaji_master, $array_kgaji_tanggal, $array_kgaji_nominal, $array_kgaji_keterangan);
		echo $result;
	}
	
	function detail_gaji_delete(){
        $kgaji_id = trim(@$_POST["kgaji_id"]); // Get our array back and translate it :
		$result=$this->m_karyawan->detail_gaji_delete($kgaji_id);
		echo $result;
    }
	...
	function karyawan_update(){
		...
		*/
		...
		$karyawan_cab_ygk=str_replace("'", '"',$karyawan_cab_ygk);
		$karyawan_cab_sjl=trim(@$_POST["karyawan_cab_sjl"]);
		$karyawan_cab_sjl=str_replace("/(<\/?)(p)([^>]*>)", "",$karyawan_cab_sjl);
		$karyawan_cab_sjl=str_replace(",", ",",$karyawan_cab_sjl);
		$karyawan_cab_sjl=str_replace("'", '"',$karyawan_cab_sjl);
		$karyawan_cab_smg=trim(@$_POST["karyawan_cab_smg"]);
		$karyawan_cab_smg=str_replace("/(<\/?)(p)([^>]*>)", "",$karyawan_cab_smg);
		$karyawan_cab_smg=str_replace(",", ",",$karyawan_cab_smg);
		$karyawan_cab_smg=str_replace("'", '"',$karyawan_cab_smg);
