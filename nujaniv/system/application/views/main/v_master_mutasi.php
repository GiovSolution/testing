...
Ext.onReady(function(){
  ...
  /* Function for Retrieve permission DataStore */
	...
	function pengecekan_dokumen(){
		...
		Ext.Ajax.request({  
			...
			params: {
				...
				mutasi_asal		: mutasi_order_gudang_asal_idField.getValue(),
				mutasi_tujuan	: mutasi_order_gudang_tujuan_idField.getValue()*/
			}, 
	...
	function master_mutasi_create(opsi){
		...
		var mutasi_tujuan_idcreate=null; 
		var mutasi_asal_idcreate=null; 
		...
		var mutasi_spb_id_create=null;

		mutasi_id_create_pk=get_pk_id();
		if(mutasi_asalField.getValue()!== null){mutasi_asal_create = mutasi_asalField.getValue();} 
		
		if(mutasi_asal_create == '1'){
			mutasi_asal_create = "Gudang Besar";
		}else if(mutasi_asal_create == '2'){
			mutasi_asal_create = "Gudang Retail";
		}else if(mutasi_asal_create == '3'){
			mutasi_asal_create = "Kabin Terapis";
		}else if(mutasi_asal_create == '4'){
			mutasi_asal_create = "Kabin Suster";
		}/*else{
			mutasi_asal_create = "Gudang Temporary";
		}*/
		
		if(mutasi_order_gudang_asal_idField.getValue()!== null){mutasi_asal_idcreate = mutasi_order_gudang_asal_idField.getValue();} 
		...
		if(mutasi_order_gudang_tujuan_idField.getValue()!== null){mutasi_tujuan_idcreate = mutasi_order_gudang_tujuan_idField.getValue();} 
		if(mutasi_tanggalField.getValue()!== ""){mutasi_tanggal_create_date = mutasi_tanggalField.getValue().format('Y-m-d');} 
		...
		if(mutasi_spb_idField.getValue()!== null){mutasi_spb_id_create = mutasi_spb_idField.getValue();}
		...
		Ext.Ajax.request({  
			...
			params: {
				...
				mutasi_asal_id			: mutasi_asal_idcreate, 
				mutasi_tujuan_id		: mutasi_tujuan_idcreate, 
				...
				mutasi_spb_id		: mutasi_spb_id_create,
			...
			success: function(response){             
				...
				if(result!==0){
						...
						cbo_mutasi_order_DataSore.load({
	...
	function master_mutasi_reset_form(){
		...
		mutasi_order_gudang_asal_idField.reset();
		mutasi_order_gudang_asal_idField.setValue(null);
		mutasi_order_gudang_tujuan_idField.reset();
		mutasi_order_gudang_tujuan_idField.setValue(null);
		...
		mutasi_spb_idField.setDisabled(false);
		mutasi_racikanFieldSet.setDisabled(true);
		mutasi_order_gudang_asal_idField.setDisabled(false);
		mutasi_order_gudang_tujuan_idField.setDisabled(false);
		...
		master_mutasi_createForm.mbPrintOnlyButton.disable();
	...
	function master_mutasi_set_form(){
		...
		mutasi_spb_idField.setValue(master_mutasiListEditorGrid.getSelectionModel().getSelected().get('mutasi_spb'));
		...
		mutasi_spbField.setValue(master_mutasiListEditorGrid.getSelectionModel().getSelected().get('omutasi_no'));
		...
		mutasi_order_gudang_asal_idField.setValue(master_mutasiListEditorGrid.getSelectionModel().getSelected().get('mutasi_asal_id'));
		mutasi_order_gudang_tujuan_idField.setValue(master_mutasiListEditorGrid.getSelectionModel().getSelected().get('mutasi_tujuan_id'));
		...
		if(mutasi_post2db=="UPDATE" && master_mutasiListEditorGrid.getSelectionModel().getSelected().get('mutasi_status')=="Terbuka"){
			...
			else
			{
				...
				//mutasi_asalField.setDisabled(false);
				//mutasi_tujuanField.setDisabled(false);
				mutasi_spb_idField.setDisabled(true);
				mutasi_tanggalField.setDisabled(false);
				mutasi_keteranganField.setDisabled(false);
				mutasi_kategori_barang_keluarField.setDisabled(false);
				mutasi_statusField.setDisabled(false);
		...
		if(mutasi_post2db=="UPDATE" && master_mutasiListEditorGrid.getSelectionModel().getSelected().get('mutasi_status')=="Tertutup"){
			...
			mutasi_button_saveprintField.setDisabled(true);
			master_mutasi_createForm.mbPrintOnlyButton.enable();
			mutasi_button_saveField.setDisabled(false);
...
*/				render: function(c){
	...
	cbo_mutasi_order_DataSore = new Ext.data.Store({
		id: 'cbo_mutasi_order_DataSore',
		proxy: new Ext.data.HttpProxy({
			url: 'index.php?c=c_master_mutasi&m=get_order_mutasi_list',
			method: 'POST'
		}),
		baseParams:{task: "LIST"}, // parameter yang di $_POST ke Controller
		reader: new Ext.data.JsonReader({
			...
			{name: 'mutasi_order_nama', type: 'string', mapping: 'omutasi_no'},
			{name: 'mutasi_order_tgl', type: 'date', dateFormat: 'Y-m-d', mapping: 'omutasi_tanggal'},
			{name: 'omutasi_asal', type: 'int', mapping: 'omutasi_asal'},
			{name: 'omutasi_tujuan', type: 'int', mapping: 'omutasi_tujuan'},
			{name: 'gudang_asal_nama', type: 'string', mapping: 'gudang_asal_nama'},
			{name: 'gudang_tujuan_nama', type: 'string', mapping: 'gudang_tujuan_nama'},
			//{name: 'omutasi_no', type: 'string', mapping: 'omutasi_no'},
		]),
		//sortInfo:{field: 'mutasi_order_nama', direction: "ASC"}
		sortInfo:{field: 'mutasi_order_tgl', direction: "DESC"}
	});

	var mutasi_order_tpl = new Ext.XTemplate(
        '<tpl for="."><div class="search-item">',
            '<span><b>{mutasi_order_nama}</b><br /></span>',
            'Tgl-Order: {mutasi_order_tgl:date("M j, Y")}<br>',
			'Supplier: {mutasi_order_supplier}',
        '</div></tpl>'
    );
	
	var mutasi_order_detail_DataStore=new Ext.data.Store({
		id: 'mutasi_order_detail_DataStore',
		proxy: new Ext.data.HttpProxy({
			url: 'index.php?c=c_master_mutasi&m=get_mutasi_order_detail_by_order_id',
			method: 'POST'
		}),
		baseParams:{task: "LIST"},
		reader: new Ext.data.JsonReader({
			...
			{name: 'domutasi_master', type: 'int', mapping: 'order_id'},
			{name: 'dmutasi_produk', type: 'int', mapping: 'domutasi_produk'},
		//	{name: 'domutasi_produk_nama', type: 'string', mapping: 'produk_nama'},
		//	{name: 'jumlah_sisa', type: 'float', mapping: 'jumlah_sisa'},
		//	{name: 'dterima_jumlah', type: 'float', mapping: 'dterima_jumlah'},
			{name: 'dmutasi_jumlah', type: 'float', mapping: 'jumlah_order'},
			{name: 'dmutasi_satuan', type: 'int', mapping: 'domutasi_satuan'},
		//	{name: 'domutasi_produk_satuan', type: 'string', mapping: 'satuan_nama'},
		//	{name: 'dterima_harga', type: 'float', mapping: 'domutasi_harga'},
		//	{name: 'dterima_diskon', type: 'float', mapping: 'domutasi_diskon'},
		//	{name: 'domutasi_produk_subtotal', type: 'float', mapping: 'subtotal'}
		]),
		sortInfo:{field: 'dmutasi_produk', direction: "ASC"}
	});
	
	/* Identify mutasi_spb Field */
	mutasi_spbField= new Ext.form.ComboBox({
		id: 'mutasi_spbField',
		fieldLabel: 'No SMB',
		store: cbo_mutasi_order_DataSore,
		displayField:'mutasi_order_nama',
		mode : 'remote',
		valueField: 'omutasi_id',
		pageSize:10,
        typeAhead: false,
		forceSelection: true,
        hideTrigger:false,
		allowBlank: false,
		tpl: mutasi_order_tpl,
	...
	label_smbField= new Ext.form.Label({ html: '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;(Untuk Klinik yang belum menggunakan SMB, field ini dapat <br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; dikosongi)' });
	
	mutasi_spb_idField= new Ext.form.NumberField();
	...
	mutasi_order_gudang_asal_idField= new Ext.form.TextField();
	mutasi_order_gudang_tujuan_idField= new Ext.form.TextField();
		...
		items:[
			{
				...
				layout: 'form',
				border:false,
				items: [mutasi_tanggalField,mutasi_noField, mutasi_spbField,label_smbField,{xtype: 'spacer',height:10}, mutasi_asalField, mutasi_tujuanField, mutasi_kategori_barang_keluarField,  mutasi_idField,mutasi_keteranganField,mutasi_statusField, mutasi_status_terimaField] 
			},
			{
				columnWidth:0.2,
				layout: 'form',
				border:false,
				labelWidth: 1,
				items: [{xtype: 'spacer',height:145},mutasi_barang_keluarField] 
			},
	...
	master_mutasi_createForm = new Ext.FormPanel({
		...
		buttons: [
			...
			<?php } ?>
			{
				...
				handler: function(){
					//master_mutasi_reset_form
					cbo_mutasi_order_DataSore.reload();
					master_mutasi_createWindow.hide();
				}
			}
		]
	...
	/*Event ketika No SMB dipilih, maka akan menginsertkan detail produk yang sudah diinput ke Detail Data Store */
	mutasi_spbField.on("select",function(){
		var j=cbo_mutasi_order_DataSore.findExact('omutasi_id',mutasi_spbField.getValue());
		...
		if(cbo_mutasi_order_DataSore.getCount()){
			mutasi_spb_idField.setValue(cbo_mutasi_order_DataSore.getAt(j).data.omutasi_id);
			mutasi_order_gudang_asal_idField.setValue(cbo_mutasi_order_DataSore.getAt(j).data.omutasi_asal);
			mutasi_order_gudang_tujuan_idField.setValue(cbo_mutasi_order_DataSore.getAt(j).data.omutasi_tujuan);
			mutasi_tujuanField.setValue(cbo_mutasi_order_DataSore.getAt(j).data.gudang_tujuan_nama); /////
			mutasi_asalField.setValue(cbo_mutasi_order_DataSore.getAt(j).data.gudang_asal_nama); /////	
		}
		
		mutasi_order_detail_DataStore.load({
			params:{orderid: mutasi_spbField.getValue()},
			callback: function(r,opt,success){
				if(success==true){
					cbo_mutasi_produkDataStore.setBaseParam('task','order');
					cbo_mutasi_produkDataStore.setBaseParam('order_id',mutasi_spbField.getValue());
					cbo_mutasi_produkDataStore.load({
						callback: function(r,opt,success){
							if(success==true){
								detail_mutasi_DataStore.removeAll();
								for(i=0;i<mutasi_order_detail_DataStore.getCount();i++){
										var detail_order_record=mutasi_order_detail_DataStore.getAt(i);
										detail_mutasi_DataStore.insert(i,detail_order_record);
								}
								detail_mutasi_total();
							}
							cbo_mutasi_satuanDataStore.setBaseParam('task','order');
							cbo_mutasi_satuanDataStore.setBaseParam('order_id',mutasi_spbField.getValue());
							cbo_mutasi_satuanDataStore.load();
						}
					});
				}
