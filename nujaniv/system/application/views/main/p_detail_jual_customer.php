<?php /* These code was generated using phpCIGen v 0.1.b (24/06/2009)#zaqi zaqi.smart@gmail.com,http:#CV. Trust Solution, jl. Saronojiwo 19 Surabaya, http://www.ts.co.id+ Module : Penjualan Print+ Description: For Print View+ Filename : p_detail_jual.php + Author : + Created on 01/Feb/2010 14:30:05*/ ?>
...
<caption>Laporan Detail Penjualan <?php echo $jenis; ?><br/><?php echo $periode; ?><br/>Group By Customer, Opsi by <?php echo $opsi; ?></caption>
<thead> 
	<tr> 
		<th scope='col'>No</th> 
...
<?php 
	...
	if ($opsi == 'Grooming') {
		$parameter_grooming = $print->cust_nama;
	} else {
		$parameter_grooming = $print->cust_no;
	}
		?><?php if($cust!==$parameter_grooming) { ?> 
			<tr> 
			...
			<?php foreach($data_print as $print_list) {
				if ($opsi == 'Grooming') {
					$parameter_grooming2 = $print_list->cust_nama;
				} else {
					$parameter_grooming2 = $print_list->cust_no;
				}
				if($parameter_grooming==$parameter_grooming2){ 
				$i++; 
	...
	<tr> 
		...
		<td align="right" class="numeric">&nbsp;</td> <td align="right" class="numeric">&nbsp;</td> 
