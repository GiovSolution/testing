...
var anam_imageSebelumField;
var anam_imageSesudahField;
...
Ext.onReady(function(){
	...
	function anamnesa_create(){
		...
		var anam_image_sebelum_create=null; 
		var anam_image_sesudah_create=null; 
		...
		if(anam_imageSebelumField.getValue()!== null){anam_image_sebelum_create = anam_imageSebelumField.getValue();} 
		if(anam_imageSesudahField.getValue()!== null){anam_image_sesudah_create = anam_imageSesudahField.getValue();} 
		...
		Ext.Ajax.request({  
			...
			params: {
				...
				anam_image_sebelum	: anam_image_sebelum_create,
				anam_image_sesudah	: anam_image_sesudah_create
	...
	function anamnesa_reset_form(){
		...
		anam_imageSebelumField.reset();
		anam_imageSebelumField.setValue(null);
		anam_imageSesudahField.reset();
		anam_imageSesudahField.setValue(null);
		anam_display_sebelumField.reset();
		anam_display_sebelumField.setValue(null);
		anam_display_sesudahField.reset();
		anam_display_sesudahField.setValue(null);
	...
	function anamnesa_set_form(){
		...
		anam_imageSebelumField.setValue(anamnesaListEditorGrid.getSelectionModel().getSelected().get('anam_image_sebelum'));
		anam_imageSesudahField.setValue(anamnesaListEditorGrid.getSelectionModel().getSelected().get('anam_image_sesudah'));
		...
		temp += '<img src="./photos/';
		temp += anam_imageSebelumField.getValue();
		temp += '" alt="Thumbnail" width="150" height="150"/>';
		anam_display_sebelumField.setValue(temp);

		temp_sesudah += '<img src="./photos/';
		temp_sesudah += anam_imageSesudahField.getValue();
		temp_sesudah += '" alt="Thumbnail" width="150" height="150"/>';
		anam_display_sesudahField.setValue(temp_sesudah);
	...
	anamnesa_DataStore = new Ext.data.Store({
		...
		reader: new Ext.data.JsonReader({
			...
			{name: 'anam_image_sebelum', type: 'string', mapping: 'anam_image_sebelum'}, 
			{name: 'anam_image_sesudah', type: 'string', mapping: 'anam_image_sesudah'}, 
	...
	// Identify Upload Image sebeleum
	anam_imageSebelumField = new Ext.form.FileUploadField({
        // renderTo: 'fi-basic',
    ...
    // Identify Upload Image sesudah
	anam_imageSesudahField = new Ext.form.FileUploadField({
        // renderTo: 'fi-basic',
        fieldLabel : 'Upload Image',
        width: 250,
        enableKeyEvents: true
	...
	// Identify tampilan image sebelum
	anam_display_sebelumField = new Ext.form.DisplayField({
		fieldLabel : 'Image Sebelum',
		height : 150,
		enableKeyEvents: true
	});

	// Identify tampilan image sesudah
	anam_display_sesudahField = new Ext.form.DisplayField({
		fieldLabel : 'Image Sesudah',
		height : 150,
		enableKeyEvents: true
	});
		...
		items:[
			{
				...
				layout: 'form',
				border:false,
				items: [anam_tanggalField, anam_petugasField, anam_custField, anam_hamilField, anam_pengobatanField, anam_perawatanField, anam_terapiField,anam_imageSebelumField,anam_display_sebelumField] 
			}
			,{
				columnWidth:0.5,
				layout: 'form',
				border:false,
				items: [anam_alergiField, anam_obatalergiField, anam_efekobatalergiField, anam_kbField, anam_harapanField,anam_idField,anam_imageSesudahField,anam_display_sesudahField] 
			}
			]
...
*/
	...
	anam_imageSebelumField.on('focus', function(){
		temp = '';
		temp += '<img src="./photos/';
		temp += anam_imageSebelumField.getValue();
		temp += '" alt="Thumbnail" width="150" height="150"/>';
	...
	anam_imageSesudahField.on('focus', function(){
		temp_sesudah = '';
		temp_sesudah += '<img src="./photos/';
		temp_sesudah += anam_imageSesudahField.getValue();
		temp_sesudah += '" alt="Thumbnail" width="150" height="150"/>';
