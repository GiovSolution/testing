...
var kas_DataStore;
var kas_ColumnModel;
var kas_ListEditorGrid;
var kas_createForm;
var kas_createWindow;
var kas_searchForm;
var kas_searchWindow;
var kas_SelectedRow;
var kas_ContextMenu;
//declare konstant
var post2db_kas = '';
...
Ext.onReady(function(){
	...
	if(oGrid_event.record.data.bank_aktif!== null){bank_aktif_update = oGrid_event.record.data.bank_aktif;}
		...
		Ext.Ajax.request({  
			waitMsg: 'Please wait...',
			url: 'index.php?c=c_kas&m=get_action',
			params: {
				task: "UPDATE",
			...
			success: function(response){							
				...
				switch(result){
					case 1:
						kas_DataStore.commitChanges();
						kas_DataStore.reload();
					...
					default:
						Ext.MessageBox.show({
							   title: 'Warning',
							   msg_kas: 'Data Kas tidak bisa disimpan.',
			...
			failure: function(response){
				...
				Ext.MessageBox.show({
							   title: 'Error',
							   msg_kas: 'Tidak bisa terhubung dengan database server',
							   buttons: Ext.MessageBox.OK,
							   animEl: 'database',
	...
	function kas_create(){
		...
		Ext.Ajax.request({  
				...
				success: function(response){             
					...
					switch(result){
						case 1:
							Ext.MessageBox.alert(post2db_kas+' OK','Data Kas berhasil disimpan.');
							kas_DataStore.reload();
							total_kas_list_DataStore.reload();
							kas_createWindow.hide();
							break;
						default:
							Ext.MessageBox.show({
							   title: 'Warning',
	...
	function get_pk_id(){
		if(post2db_kas=='UPDATE')
			return kas_ListEditorGrid.getSelectionModel().getSelected().get('kas_id');
		else 
			return 0;
	}
	/* End of Function  */
	...
	function kas_set_form(){
		kas_tanggalField.setValue(kas_ListEditorGrid.getSelectionModel().getSelected().get('kas_tanggal'));
		kas_jumlahField.setValue(CurrencyFormatted(kas_ListEditorGrid.getSelectionModel().getSelected().get('kas_jumlah')));
		kas_keteranganField.setValue(kas_ListEditorGrid.getSelectionModel().getSelected().get('kas_keterangan'));
		kas_tipeField.setValue(kas_ListEditorGrid.getSelectionModel().getSelected().get('kas_tipe'));
	}
	...
	function display_form_window(){
		if(!kas_createWindow.isVisible()){
			
			post2db_kas='CREATE';
			msg_kas='created';
			kas_reset_form();
			
			kas_createWindow.show();
		} else {
			kas_createWindow.toFront();
		}
	}
  	/* End of Function */
	...
	function kas_confirm_update(){
		...
		if(kas_ListEditorGrid.selModel.getCount() == 1) {
			...
			Ext.MessageBox.show({
				title: 'Warning',
				msg_kas: 'Tidak ada data yang dipilih untuk diedit',
				buttons: Ext.MessageBox.OK,
				animEl: 'save',
				icon: Ext.MessageBox.WARNING
			});
	...
	function bank_delete(btn){
		if(btn=='yes'){
			var selections = kas_ListEditorGrid.selModel.getSelections();
			var prez = [];
			for(i = 0; i< kas_ListEditorGrid.selModel.getCount(); i++){
				...
				success: function(response){
					...
					switch(result){
						...
						default:
							Ext.MessageBox.show({
								title: 'Warning',
								msg_kas: 'Tidak bisa menghapus data yang diplih',
								buttons: Ext.MessageBox.OK,
								animEl: 'save',
								icon: Ext.MessageBox.WARNING
							});
	...
	kas_DataStore = new Ext.data.Store({
		id: 'kas_DataStore',
		proxy: new Ext.data.HttpProxy({
			url: 'index.php?c=c_kas&m=get_action', 
			method: 'POST'
		}),
		baseParams:{task: "LIST", start: 0, limit: pageS_kas}, // parameter yang di $_POST ke Controller
		reader: new Ext.data.JsonReader({
			root: 'results',
			totalProperty: 'total',
			id: 'kas_id'
		},[
			{name: 'kas_id', type: 'int', mapping: 'kas_id'},
			{name: 'kas_tanggal', type: 'date', dateFormat: 'Y-m-d', mapping: 'kas_tanggal'},
			{name: 'kas_jumlah', type: 'float', mapping: 'kas_jumlah'},
			{name: 'kas_keterangan', type: 'string', mapping: 'kas_keterangan'},
			{name: 'kas_tipe', type: 'string', mapping: 'kas_tipe'},
			{name: 'kas_creator', type: 'string', mapping: 'kas_creator'},
			{name: 'kas_date_create', type: 'date', dateFormat: 'Y-m-d H:i:s', mapping: 'kas_date_create'},
			{name: 'trucking_update', type: 'string', mapping: 'trucking_update'},
			{name: 'kas_date_update', type: 'date', dateFormat: 'Y-m-d H:i:s', mapping: 'kas_date_update'},
	...
	cbo_bank_akunDataStore = new Ext.data.Store({
		...
		proxy: new Ext.data.HttpProxy({
			url: 'index.php?c=c_kas&m=get_akun_list', 
			method: 'POST'
		}),
			reader: new Ext.data.JsonReader({
			root: 'results',
	...
	proxy: new Ext.data.HttpProxy({
			url: 'index.php?c=c_kas&m=get_mbank_list', 
			method: 'POST'
		}),
			reader: new Ext.data.JsonReader({
			root: 'results',
	...
	total_kas_list_DataStore = new Ext.data.GroupingStore({
		...
		proxy: new Ext.data.HttpProxy({
			url: 'index.php?c=c_kas&m=saldo_akhir_list', 
			method: 'POST'
		}),
		baseParams:{task: "LIST",start:0,limit:pageS_kas}, // parameter yang di $_POST ke Controller
		reader: new Ext.data.JsonReader({
	...
	kas_ColumnModel = new Ext.grid.ColumnModel(
		[{
			header: '#',
			readOnly: true,
			dataIndex: 'kas_id',
	...
	kas_ColumnModel.defaultSortable= true;
	/* End of Function */
	...
	kas_ListEditorGrid =  new Ext.grid.EditorGridPanel({
		id: 'kas_ListEditorGrid',
		el: 'fp_kas',
		title: 'Kas Masuk/Keluar',
		autoHeight: true,
		store: kas_DataStore, // DataStore
		cm: kas_ColumnModel, // Nama-nama Columns
		enableColLock:false,
		frame: true,
		...
		/*{
			...
			store: kas_DataStore,
			params: {task: 'LIST',start: 0, limit: pageS_kas},
			listeners:{
				specialkey: function(f,e){
					if(e.getKey() == e.ENTER){
						kas_DataStore.baseParams={task:'LIST',start: 0, limit: pageS_kas};
		            }
				},
				render: function(c){
	...
	kas_ListEditorGrid.render();
	/* End of DataStore */
     
	/* Create Context Menu */
	kas_ContextMenu = new Ext.menu.Menu({
	...
	function onkas_ListEditGridContextMenu(grid, rowIndex, e) {
		e.stopEvent();
		var coords = e.getXY();
		kas_ContextMenu.rowRecord = grid.store.getAt(rowIndex);
		grid.selModel.selectRow(rowIndex);
		kas_SelectedRow=rowIndex;
		kas_ContextMenu.showAt([coords[0], coords[1]]);
  	}
  	/* End of Function */
	
	/* function for editing row via context menu */
	function kas_editContextMenu(){
      kas_ListEditorGrid.startEditing(kas_SelectedRow,1);
  	}
	/* End of Function */
  	
	kas_ListEditorGrid.addListener('rowcontextmenu', onkas_ListEditGridContextMenu);
	kas_DataStore.load({params: {start: 0, limit: pageS_kas}});	// load DataStore
	total_kas_list_DataStore.load();
	kas_ListEditorGrid.on('afteredit', bank_update); // inLine Editing Record
	...
	kas_createForm = new Ext.FormPanel({
		labelAlign: 'left',
		...
		items: [{
			...
			items:[
			{
				...
				layout: 'form',
				border:false,
				items: [kas_tanggalField, kas_jumlahField, kas_keteranganField, kas_tipeField] 
			}
			]
		}]
		,
	...
	kas_createWindow= new Ext.Window({
		id: 'kas_createWindow',
		title: post2db_kas+'Kas',
		closable:true,
		closeAction: 'hide',
		Width: 600,
		autoHeight: true,
	...
	function kas_list_search(){
		...
		kas_DataStore.reload({params: {start: 0, limit: pageS_kas}});
	}
		
	/* Function for reset search result */
	function kas_reset_search(){
		// reset the store parameters
		kas_DataStore.baseParams = { task: 'LIST' };
		// Cause the datastore to do another query : 
		kas_DataStore.reload({params: {start: 0, limit: pageS_kas}});
		total_kas_list_DataStore.reload({params: {start: 0, limit: pageS_kas}});
		//kas_searchWindow.close();
	};
	/* End of Fuction */
	...
	kas_searchForm = new Ext.FormPanel({
		labelAlign: 'left',
		bodyStyle:'padding:5px',
		autoHeight:true,
		width: 300,        
	...
	kas_searchWindow = new Ext.Window({
		title: 'Pencarian Kas',
		closable:true,
		closeAction: 'hide',
		autoWidth: true,
		...
		items: kas_searchForm
	});
    /* End of Function */ 
	 
  	/* Function for Displaying  Search Window Form */
	function display_form_search_window(){
		if(!kas_searchWindow.isVisible()){
			bank_reset_SearchForm();
			kas_searchWindow.show();
		} else {
			kas_searchWindow.toFront();
		}
	}
  	/* End Function */
	...
	function kas_print(){
		...
		if(kas_DataStore.baseParams.query!==null){searchquery = kas_DataStore.baseParams.query;}
		if(kas_DataStore.baseParams.bank_kode!==null){bank_kode_print = kas_DataStore.baseParams.bank_kode;}
		if(kas_DataStore.baseParams.bank_nama!==null){bank_nama_print = kas_DataStore.baseParams.bank_nama;}
		if(kas_DataStore.baseParams.bank_norek!==null){bank_norek_print = kas_DataStore.baseParams.bank_norek;}
		if(kas_DataStore.baseParams.bank_atasnama!==null){bank_atasnama_print = kas_DataStore.baseParams.bank_atasnama;}
		if(kas_DataStore.baseParams.bank_saldo!==null){bank_saldo_print = kas_DataStore.baseParams.bank_saldo;}
		if(kas_DataStore.baseParams.bank_keterangan!==null){bank_keterangan_print = kas_DataStore.baseParams.bank_keterangan;}
		if(kas_DataStore.baseParams.bank_aktif!==null){bank_aktif_print = kas_DataStore.baseParams.bank_aktif;}
		...
		params: {
			...
			bank_aktif : bank_aktif_print,
		  	currentlisting: kas_DataStore.baseParams.task // this tells us if we are searching or not
		}, 
		success: function(response){              
		  	var result=eval(response.responseText);
		  	switch(result){
		  	...
		  	default:
				Ext.MessageBox.show({
					title: 'Warning',
					msg_kas: 'Tidak bisa mencetak data!',
					buttons: Ext.MessageBox.OK,
					animEl: 'save',
					icon: Ext.MessageBox.WARNING
				});
	...
	function kas_export_excel(){
		...
		// check if we do have some search data...
		if(kas_DataStore.baseParams.query!==null){searchquery = kas_DataStore.baseParams.query;}
		if(kas_DataStore.baseParams.bank_kode!==null){bank_kode_2excel = kas_DataStore.baseParams.bank_kode;}
		if(kas_DataStore.baseParams.bank_nama!==null){bank_nama_2excel = kas_DataStore.baseParams.bank_nama;}
		if(kas_DataStore.baseParams.bank_norek!==null){bank_norek_2excel = kas_DataStore.baseParams.bank_norek;}
		if(kas_DataStore.baseParams.bank_atasnama!==null){bank_atasnama_2excel = kas_DataStore.baseParams.bank_atasnama;}
		if(kas_DataStore.baseParams.bank_saldo!==null){bank_saldo_2excel = kas_DataStore.baseParams.bank_saldo;}
		if(kas_DataStore.baseParams.bank_keterangan!==null){bank_keterangan_2excel = kas_DataStore.baseParams.bank_keterangan;}
		if(kas_DataStore.baseParams.bank_aktif!==null){bank_aktif_2excel = kas_DataStore.baseParams.bank_aktif;}
		...
		success: function(response){              
		  	...
		  	default:
				Ext.MessageBox.show({
					title: 'Warning',
					msg_kas: 'Tidak bisa meng-export data ke dalam format excel!',
					buttons: Ext.MessageBox.OK,
					animEl: 'save',
					icon: Ext.MessageBox.WARNING
				});
